          ...
          <motion.div variants={fadeInUp} className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">إدارة الدروس والمحتوى التعليمي</p>
              </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="title_ar" value={formData.title_ar} onChange={(e) => setFormData({ ...formData, title_ar: e.target.value })} placeholder="أدخل عنوان الدرس بالعربية" />
        </div>
        <div className="space-y-2">
          <Label htmlFor="title">عنوان الدرس (إنجليزي)</Label>
          <Input id="title" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Enter lesson title in English" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={formData.difficulty_level.toString()} onValueChange={(value) => setFormData({ ...formData, difficulty_level: Number.parseInt(value) })}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          ...
          <Input id="points" type="number" value={formData.points_reward} onChange={(e) => setFormData({ ...formData, points_reward: Number.parseInt(e.target.value) || 0 })} min="1" max="100" />
        </div>
      </div>
        ...
        {formData.content.sections.map((section, index) => (
          <Card key={index} className="p-4">
            ...
            <div className="space-y-3">
              <Input placeholder="عنوان القسم (اختياري)" value={section.title || ""} onChange={(e) => updateContentSection(index, "title", e.target.value)} />

              {section.type === "text" && (
                <Textarea placeholder="محتوى النص..." value={section.content} onChange={(e) => updateContentSection(index, "content", e.target.value)} rows={4} />
              )}

              {(section.type === "video" || section.type === "image") && (
                <Input placeholder={section.type === "video" ? "رابط الفيديو أو مسار الملف" : "رابط الصورة أو مسار الملف"} value={section.content} onChange={(e) => updateContentSection(index, "content", e.target.value)} />
              )}
            </div>
          </Card>
        ))}
