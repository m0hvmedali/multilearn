...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const averageScore = progressData && progressData.length > 0 ? progressData.reduce((sum, p) => sum + (p.score || 0), 0) / progressData.length : 0

      setSubjects(subjectsData || [])
      setRecentLessons(lessonsData || [])
      setUserProgress(progressData || [])
      setAchievements(achievementsData || [])
      setStats({ totalLessons, completedLessons, totalTimeSpent, averageScore })
    } catch (error) {
        ...
        <motion.div initial="initial" animate="animate" variants={staggerContainer}>
          <motion.div variants={fadeInUp} className="mb-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-primary mb-2">مرحباً، {profile?.full_name || user.email}!</h1>
                <p className="text-muted-foreground">استمر في رحلتك التعليمية واكسب المزيد من النقاط</p>
              </div>
              <div className="text-left">
                <div className="text-2xl font-bold text-accent">{profile?.points || 0} نقطة</div>
                <div className="text-sm text-muted-foreground">المستوى {profile?.level || 1}</div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeInUp} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-accent">{formatTime(stats.totalTimeSpent)}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-accent">{Math.round(stats.averageScore)}%</div>
                          ...
                          <Card key={lesson.id} className="hover:shadow-md transition-shadow">
                            <CardHeader className="pb-3">
                              <div className="flex items-start justify-between">
                                ...
                                {isCompleted && <CheckCircle className="h-4 w-4 text-accent flex-shrink-0" />}
                              </div>
                            </CardHeader>
                    ...
                    {achievements.length > 0 ? (
                      <div className="space-y-4">
                        {achievements.map((achievement) => (
                          <div
                            key={achievement.id}
                            className="flex items-center gap-4 p-4 bg-accent/10 rounded-lg border border-accent/30"
                          >
